...
state containing deduplication, progress, and failure bookkeeping. Preserve it during ongoing work; Linear/GitHub being canonical does not authorize wiping it.
...
bun run scripts/probe-relations.ts       # print blocks/related relations + derived blockers per assignment
...
- **Action cache filters by `success = 1`**. Failed actions remain retryable. The circuit breaker counts consecutive failed paid handling on the current fingerprint (default 5 in 6h), not successful interactions, holds, pre-request stale-input deferrals or quota pauses. Failed intent classification, code, questions and nudge generation all count because they can spend tokens. New input/head or a completed handling action resets the streak. Mention-only questions use the same limit without changing the issue assignee. `failure_kind` is an additive SQLite migration; old quota rows are recognized by their existing error prefix.
...
- **Revisit marks gate `revisit_code`**: `ticket_revisit_marks` stores the `humanInputSignature` Gary acted on when `start_coding` ran; `revisit_code` only fires when the current signature differs. Pre-feature tickets without a mark stay dormant by design — back-filling the mark to "current" would silently drop comments Gary missed. Linear follow-ups reconstruct the actually handled comment prefix from that mark and process every new comment. Explicit code requests use the same PR branch; questions get a read-only tool phase. Unknown boundaries (for example a description edit) get a clarification, never a replayed old command. Both comment reads paginate the same bounded complete snapshot. Marks advance only after successful handling and reply. See `src/state/queries.ts:getRevisitMark`.
...
- **Blocked tickets hold instead of coding**: `start_coding` is gated on Linear "blocked by" relations (`inverseRelations` with `type: "blocks"` in the issue fetch). An open blocker (state not completed/canceled — unknown/inaccessible counts as open, fail closed) swaps the emitted action to `wait_for_blocker`, which posts one hold comment (voice.md example 18) and goes dormant. Blocker state is deliberately NOT in `fingerprintDerivedState` — adding a canonical field would shift every existing fingerprint and dump the action cache. `wait_for_blocker` is also excluded from `countConsecutiveWorkFailures` so humans discussing a blocked ticket can't trip the circuit breaker. Only `start_coding` is gated: classify/answer/bounce and all existing-PR actions proceed regardless of blockers.
- **Beads awareness is detection-based**: `loadProjectContext` sets `hasBeads` when the workspace has a `.beads/` directory (mulligan-labs repos do), and `formatProjectContext` injects a read-only note — agents may read `.beads/issues.jsonl` for context but must never mutate beads state or commit changes under `.beads/`. Linear stays the ticket source of truth; a Gary↔beads state seam remains a §16-style non-goal.
- **Scope=L no longer auto-bounces** — removed in `4a5dac1` ("defaults to ambition", voice.md). `decideClassifyOutcome` only enforces the confidence floor; scope is still recorded. The backstop for oversized tickets is now in `code.ts`: a loop that hits iteration_cap/timeout with no commits escalates to the reporter (ERT-1648 burned 617k input tokens this way pre-removal; ERT-2353 hit the same cap 2026-08-15 and escalated cleanly). If L tickets start burning caps routinely, revisit — the removal traded token waste for ambition deliberately.
...
`bun run deploy` is the canonical update path — it pulls latest from `707-Labs/gary` (public; never commit credentials or private work context), installs, typechecks, syncs the plist if it changed, reloads launchd. The script uses `git pull --ff-only` so it refuses to advance if the mini has local changes.
...
Existing-PR follow-ups refuse dirty, divergent, switched, or closed PR state and preserve local work. Missing worktrees are restored without resetting existing branches. Follow-up pushes are ordinary fast-forward pushes; concurrent remote updates reject the push. Failed replies remain pending. CI retry limits count completed CI repair attempts only, excluding quota waits and the current in-flight attempt. Initial coding still has its separate no-commit escalation path; this change does not add automatic continuation for that path.
