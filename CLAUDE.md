...
`GLMClient.providersUsed()`, including fixup rounds) sink to the back
of the reviewer's preferred order (`decorrelatedOrder` in
`src/providers.ts`). Static config can't guarantee this — fallback
landed both roles on deepseek for ERT-1924 and two review rounds
shared the author's blind spots. Soft preference only: if arming
leaves just an author provider, review still runs on it and a
`review_correlated_provider` event is recorded.
...
untested changed logic, security defect). The prompt carries a
security checklist (sanitize-last ordering, raw HTML sinks,
parameterized SQL, auth parity, framework error typing) and lists the
target repo's rule docs by path. Style and refactor opinions go in
...
`.env` required keys: `LINEAR_API_KEY`, `GARY_LINEAR_USER_ID`, `LINEAR_TEAM_ID`, `LINEAR_IN_PROGRESS_STATE_ID`, `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY_PATH` (or inline `GITHUB_APP_PRIVATE_KEY`), `GITHUB_APP_INSTALLATION_ID`, `Z_AI_API_KEY`, `GARY_REPO_MAP`. Optional providers: `KIMI_API_KEY` (Kimi Code, defaults to `https://api.kimi.com/coding` + `k3` — the default main-work primary), `DEEPSEEK_API_KEY` (defaults to `https://api.deepseek.com/anthropic` + `deepseek-v4-pro`). Provider routing (optional): `GARY_MAIN_PROVIDER_ORDER` (default `kimi,z.ai,deepseek`) and `GARY_PR_FOLLOWUP_PROVIDER_ORDER` (default `z.ai,kimi,deepseek`). Optional: `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` to enable Workers Observability tools (`query_cloudflare_logs`, `list_cloudflare_invocations`) inside the agent loop. `GARY_ALLOWLISTED_MENTION_USER_IDS` (comma-separated Linear user ids) opts into the @mention pipeline — empty (default) disables it. Reviewer pass env vars (all optional): `GARY_REVIEWER_PROVIDER_ORDER`, `GARY_REVIEW_MAX_ROUNDS`, `GARY_REVIEW_ITERATION_CAP`, `GARY_REVIEW_TIMEOUT_MS`. Gate tuning (optional): `GARY_TEST_COMMAND` (post-finish test gate command; unset = autodetect, `off` = disable), `GARY_PHASE_BUDGET_{S,M,L}` (`"<investigate>/<implement>"` iteration budgets). See `.env.example`.
...
- **Backlog is a staging gate, `@mention` overrides it**: `fetchAssignedIssues` skips `completed`/`canceled`/`backlog` (`isSkippedStateType` in `src/adapters/linear.ts`, tunable via `GARY_SKIP_STATE_TYPES`; completed/canceled are always skipped). Before this, assignment was the ONLY gate — decomposing an epic into a dependency chain and parking dependents in Backlog did nothing, and Gary pulled all of them into In Progress at once to build against a main missing their predecessors' code. `fetchMentionedIssues` is deliberately NOT filtered, so "park in Backlog, `@gary` when the predecessor lands" is the intended staging workflow. Triage is still worked: tickets land in Triage unassigned, so Gary holding one means a human assigned it. Unknown/missing state types fail open.
- **`maxInFlight` governs dispatch, not provider count**: `dispatchSlots` in `src/loop.ts` bounds a tick by `min(unarmed providers, candidates, GARY_MAX_IN_FLIGHT)` (default 3). Provider count alone caps concurrency but not appetite — with 3 unarmed providers Gary opened 3 tickets a tick through the whole assigned set and tripped Kimi's quota; once gates arm, unarmed drops to 0 and throughput stops dead.
...
- **Pre-push hook runs `bun run check`** in the Ertai repo. `code.ts` re-runs the same command after the agent finishes — plus the repo's unit tests (autodetected: `test:run` first, then a non-watch `test` script; `GARY_TEST_COMMAND` forces or `off` disables — see `resolveTestGateCommand`). If a gate fails, a fix-up agent loop (15 iter cap) gets the failure output as a new task and tries to recover. If it still fails, Gary escalates with a tail of the output instead of letting the push hook reject the branch with no recourse. The post-rebase recheck stays typecheck-only (it's a speed check, and the pre-rebase tree already passed tests). See `ensurePostFinishCheckPasses` in `src/handlers/code.ts`.
...
- **Project context auto-load**: every coding handler prepends CLAUDE.md / AGENTS.md / `.claude/skills/*/SKILL.md` frontmatter from the worktree to the agent's task message, plus rule docs by path (`DESIGN.md` / `SECURITY.md` / `CONTRIBUTING.md` / `.claude/rules/*.md` via `loadRuleDocs`) — the reviewer task lists the same docs. The agent loads skill and rule-doc bodies on demand via `read_file`. Implemented in `src/skills.ts`. Regex parsing uses `String.match` not `RegExp.exec` to dodge the same security-scan false positive as `Executor.run`.
- **Comment-based fingerprints excise non-human writes**: `humanInputSignature` (Linear) hashes non-Gary comment ids; `prCommentSignature` (GitHub) hashes `User`-typed comments plus allowlisted review bots (`RESPONDABLE_BOT_LOGINS` — currently just `gemini-code-assist[bot]`); all other bots are filtered entirely (Gary, linear[bot] linkbacks, dependabot, etc.). The pr-review handler uses the SAME predicate (`isRespondablePrCommentAuthor`) — if the two filters diverge, the handler loops or goes dead. Without this, ANSWER and PR-review tickets would loop because Gary's own response bumps `updatedAt` / adds a comment and shifts the cache. PR review responses are also tracked in `pr_comment_responses` so the signature returns "empty" once Gary has answered every pending human comment. See `src/state-fingerprint.ts`.
...
- **LLM provider routing is per action type**: main work (classify, start_coding, revisit, answers) prefers Kimi K3; PR follow-ups (fix_ci_failure, respond_to_pr_review, nudge_reviewer) prefer Z.ai GLM-5.2; the reviewer pass prefers DeepSeek and is decorrelated at runtime from whichever providers authored the diff (see Reviewer pass). See `providerOrderForAction` in `src/loop.ts` and `loadProviderRoutingConfig` in `src/config.ts`. All routed chains share provider gate objects, so a 429 on one arms it everywhere. **K3 is a reasoning model** — it emits a thinking block before text, so single-turn `complete()` calls need max_tokens headroom (title/body/classifier budgets were raised for this; don't add new sub-1024 completions on the main path).
- **LLM provider chain fallback**: `GLMClient` owns a `ProviderChain` (`src/providers.ts`). `complete()` and `createMessage()` route through the highest-priority unarmed provider; on a 429 they arm that provider's gate and fall through to the next. A 401/403 (expired or revoked key) also falls through, arming the provider for `AUTH_FAILURE_BACKOFF_MS` (6h) with an error-level log — a dead fallback key must never kill a run. Other errors (5xx, malformed) still propagate. Per-provider `parse429` extracts a reset timestamp when possible (Z.ai's `code=1308` shape; Kimi a permissive ISO matcher; DeepSeek always null since it has no documented cap). Falls back to `defaultBackoffMs` (60s default) when no parse. `AllProvidersExhaustedError` only fires when every provider is armed; the tick catches it, emits `rate_limit_skip`, records the action as a non-bouncing failure so it retries with the same fingerprint when a provider clears. Tests live in `test/glm-fallback.test.ts` + `test/providers.test.ts`. **Note Kimi Code ≠ Moonshot Open Platform** — different billing surface, different endpoint (`api.kimi.com/coding` vs `api.moonshot.ai/anthropic`).
- **Scope=L no longer auto-bounces** (removed in `4a5dac1`; Gary now defaults to ambition). Phase budgets (investigate/implement iterations) are S 12/25, M 20/45, L 20/50 — raised from 8/20 and 15/35 after S-classified tickets burned the whole investigate budget on multi-file discovery and bounced. Env-tunable via `GARY_PHASE_BUDGET_{S,M,L}="<inv>/<impl>"`; L also gets 2x the loop timeout — see `phaseBudget` and `L_TIMEOUT_MULTIPLIER` in `src/handlers/code.ts`. The token-burn backstops: iteration caps, loop timeout, circuit breaker (5 attempts/6h), and the reviewer pass. ERT-1648 (617k input tokens) is the cautionary tale if these ever get loosened.
- **PR-open preflight guard**: right before `openPullRequest`, the code handler re-fetches the Linear ticket (abort if completed/canceled or no longer assigned to Gary) and lists the repo's open PRs from ANY author (abort if one already references the ticket id in head ref or title). Added after ERT-1891 got byte-identical PRs from two Gary daemons two minutes after the ticket went Done. Fails open on API errors; skips record a `pr_open_skipped` event. See `preflightPrOpen` in `src/handlers/code.ts`.
- **Branch names carry the classified change type**: the classifier emits a conventional-commit `type` (stored in `tickets.classification_type`) and branches become `fix/ert-1891-<slug>`. Missing/unknown type falls back to the legacy `ERT-1891-<slug>` format — never fail a run over branch naming. PR bodies are also sanitized post-model (`sanitizePrBody`): anything after the `🤖 Generated by` signature line is dropped.
- **Bounce/escalate resets the board**: every hand-off path (classifier bounce, `escalate()`, and the code handler's `reassignToReporter`) moves the ticket back to the team's `unstarted` state before reassigning — a ticket Gary moved to In Progress must not stay there after he drops it. Best-effort; failures log and continue.
