...
- **Scope=L no longer auto-bounces** (removed in `4a5dac1`; Gary now defaults to ambition). L tickets get a bigger phase budget (20/50 vs M's 15/35) and 2x the loop timeout — see `phaseBudget` and `L_TIMEOUT_MULTIPLIER` in `src/handlers/code.ts`. The token-burn backstops that made the old auto-bounce safe to remove: iteration caps, loop timeout, circuit breaker (5 attempts/6h), and the reviewer pass. ERT-1648 (617k input tokens) is the cautionary tale if these ever get loosened.
