...
- **Action cache filters by `success = 1`**. Failed actions remain retryable. The circuit breaker counts consecutive failed work on the current fingerprint (default 5 in 6h), not successful interactions, holds, read-only failures or quota pauses. New input/head or a completed work/follow-up action resets the streak. `failure_kind` is an additive SQLite migration; old quota rows are recognized by their existing error prefix.
